# Spec: Asma Iqbal's portfolio site

Build a one-page personal portfolio: `index.html` and `style.css`, in `site/`.
...
This is a project-specific copy of the course's generic `portfolio-starter` spec
(`panaversity/agentfactory-labs`, `crash-course/loop-eng/portfolio-starter`), adapted for
one concrete case: refining Asma Iqbal's existing live portfolio at
**https://asmaiqbal.vercel.app** rather than building from a blank page. Part A (the 20
mechanical checks, M1–M20) and Part B (the 6 judgment promises, J1–J6) are kept **verbatim**
— `check.py`'s logic was not touched, so the promises this file makes must still match it
exactly. What's adapted is the framing around them, documented below.

## Adaptations for this repo

- **Source of truth is a live site, not a CV/PDF.** The upstream spec's Phase 0
  (`profile-extractor`) expects a PDF or LinkedIn export. Here, the source is the HTML of
  the deployed site, fetched and read directly — `site/profile.md` was built from that, not
  from a résumé. J6 ("it could not have been a PDF") still applies at full strength: the
  equivalent failure here would be shipping the same page back with a fresh coat of CSS and
  no new design decision, which is exactly what J6 exists to catch regardless of source
  format.
- **`check.py` and `render.sh` hardcode a macOS-only Chrome path** (`/Applications/Google
  Chrome.app/...`). On this Windows machine that path doesn't exist, so `os.path.exists(CHROME)`
  would silently be `False` and M12, M14, and M16 would fail on *every* run regardless of
  page quality — not because the page is wrong, but because the OS check for whether Chrome
  exists never fires. Both scripts now try a short list of common install paths (macOS,
  Windows, Linux) and use whichever one actually exists. The check *logic* (render, measure,
  probe for overflow) is untouched.
- **The same file also fixed a `file://` URI bug that only shows up off macOS/Linux.**
  `f"file://{os.path.abspath(D)}"` produces a Windows path with backslashes
  (`file://E:\...\index.html`), which Chrome cannot parse as a URL. `check.py` now builds
  the URI with `pathlib.Path(...).resolve().as_uri()`, which is the stdlib-correct way to do
  this on any OS. Same reasoning as above: this makes an existing check *runnable*, it does
  not change what it accepts.

- **File reads/writes now specify `encoding="utf-8"`.** Without it, Python on Windows opens
  text files with the system codepage (cp1252 here), which cannot decode the em dashes and
  arrows (`—`, `→`) any real portfolio page's copy uses — `check.py` crashed on
  `profile.md` before a single check ran. macOS/Linux default to UTF-8, so this was invisible
  there.
- **Every headless Chrome call now passes a throwaway `--user-data-dir`.** With none set, a
  `--headless` launch that finds this machine's regular Chrome already open silently attaches
  to that session instead of starting an isolated process ("Opening in existing browser
  session") — every `--headless`/`--dump-dom`/`--screenshot` flag is then ignored, and M12,
  M14, and M16 fail across the board with no indication why. A private profile per run forces
  a real isolated instance regardless of what else is open.
- **The M14 and M16 probes now also pass `--hide-scrollbars`** (M12's render call already
  did). Without it, a classic (non-overlay) Windows scrollbar consumes ~15px of layout width
  inside the probe's headless window, so a perfectly non-overflowing page measured
  `scrollWidth=375` against a `390`px viewport and failed M16's exact-equality check for a
  reason that had nothing to do with the page. macOS's overlay scrollbars don't reserve that
  space, so the upstream script never needed the flag there.
- **The M12 screenshot path is now absolute.** Chrome's own `--screenshot=` writer fails to
  resolve a relative, forward-slash path (`site/shot.png`) on Windows — `Failed to write file
...
- **`.gitignore` no longer excludes `site/profile.md`.** The upstream file assumes this
  ...
  `design.md`, and `content.md` are the actual work product, so they're meant to be committed,
  not treated as local scratch state.

Neither adaptation touches what M1–M20 or J1–J6 actually require. Every check below still
has to be earned by the page.
...
|---|---|---|---|---|---|
| 0 | Ground truth | `profile-extractor` | the live site at asmaiqbal.vercel.app | `profile.md` | it matches the template above |
