...
# Adapted for this repo: os.path.abspath() + an f-string produced backslash "file://"
# URIs on Windows (e.g. file://E:\...\index.html), which Chrome cannot parse. pathlib's
# as_uri() is the stdlib-correct, cross-platform way to build a file:// URI.
def file_uri(path):
...
# Adapted for this repo: the upstream path is macOS-only. Without a real path here,
# M12/M14/M16 would silently fail on every OS but macOS regardless of page quality —
# not a weakened check, just the same check made runnable on this machine. See spec.md
...
prof = open(f"{D}/profile.md", encoding="utf-8").read()
...
css  = open(f"{D}/style.css", encoding="utf-8").read()
...
# Adapted for this repo: with no --user-data-dir, a headless launch that finds an already
# running Chrome on the same machine silently attaches to that session instead of starting
# an isolated headless process ("Opening in existing browser session") — every --headless
# flag below is then ignored, so M12/M14/M16 fail regardless of the page. A private,
# throwaway profile per run forces a real isolated instance on any OS.
import tempfile
_CHROME_PROFILE = tempfile.mkdtemp(prefix="check_py_chrome_")
_USER_DATA_DIR = f"--user-data-dir={_CHROME_PROFILE}"

# --- M12: render
console_err, shot_ok = [], False
if os.path.exists(CHROME):
    # Adapted for this repo: Chrome's own --screenshot writer fails to resolve a relative
    # forward-slash path on Windows ("Failed to write file site/shot.png: The system cannot
    # find the path specified") even though the same string works fine as a Python path.
    # An absolute, OS-native path sidesteps it.
...
if os.path.exists(CHROME):
    ...
    open(frame, "w", encoding="utf-8").write("""<!doctype html><meta charset=utf-8>
...
if os.path.exists(CHROME):
 ...
 document.title = all.length ? ('CPL='+Math.min.apply(null,all)+'-'+Math.max.apply(null,all)) : 'CPL=none';
    ...
    open(mp, "w", encoding="utf-8").write(_h)
